...
class ReadBilling(models.Model):
    ...
    def _generate_pdf_attachment_for_account(self, acc_number):
        ...
        report_xml_id = 'business_company.final_report_pdf'  # Таны тайлангийн XML ID

        report = self.env.ref(report_xml_id)
        pdf_content, _ = report.sudo()._render_qweb_pdf([billing_rec.id])

        # Base64-ийг зөв хөрвүүлэх
    ...
    def email_campaign_bills(self):
        ...
        for group in mail_groups:
            ...
            if not email or "@" not in email or not acc_number:
                skipped_count += 1
                _logger.warning("Хүчингүй email эсвэл acc_number skipped: ID=%s, Email=%s, Acc=%s", group.id, email, acc_number)
                continue
